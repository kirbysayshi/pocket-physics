...
stiffness) {
  ...
  var deltaMag = (0, _v.magnitude)(delta); // Difference between current distance and goal distance
...
var edgeDir = (0, _v.v2)();
var hypo = (0, _v.v2)();
var edgePointingToCircleDirection = (0, _v.v2)();

function projectPointEdge(point, endpoint1, endpoint2, result) {
  // Edge direction (edge in local space)
  (0, _v.sub)(edge, endpoint1, endpoint2); // Normalize collision edge (assume collision axis is edge)

  (0, _v.normalize)(edgeDir, edge); // We know which way the edges were wound, so we implicitly know which order
  // these points should be used in to compute the normal.

  (0, _v.normal)(result.edgeNormal, endpoint1, endpoint2); // Vector from endpoint1 to particle

  (0, _v.sub)(hypo, point, endpoint1); // Where is the particle on the edge, before, after, or on?
  // Also used for interpolation later.

  var projection = (0, _v.dot)(edge, hypo);
  var maxDot = (0, _v.dot)(edge, edge);
  var edgeMag = Math.sqrt(maxDot); // Projection is beyond the enpoints!
  ...
  var t = projection / maxDot;
  var u = 1 - t; // Find the point of projection on the edge.

  (0, _v.scale)(result.projectedPoint, edgeDir, t * edgeMag);
  (0, _v.add)(result.projectedPoint, result.projectedPoint, endpoint1);
  (0, _v.sub)(edgePointingToCircleDirection, point, result.projectedPoint); // both the edge normal and the segment from edge to circle are
  // facing a similar direction

  var similarity = (0, _v.dot)(edgePointingToCircleDirection, result.edgeNormal);
  var pointToProjectionDistance = (0, _v.distance)(point, result.projectedPoint);
  ...
  (function step() {
    ...
    for (var i = 0; i < bucket.length; i++) {
      ...
      for (var j = 0; j < circles.length; j++) {
        ...
        if (projection.similarty > 0) continue; // If we get here, the directions so dissimilar that the circle must
