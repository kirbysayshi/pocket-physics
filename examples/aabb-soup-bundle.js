...
for (var count = 30, i = 0; i < count; i++) {
  var centerX = cvs.width / 2;
  var centerY = cvs.height / 2;
  var _distance = Math.min(centerX, centerY) * 0.5;
  var x = centerX + Math.cos(i) * _distance;
  var y = centerY + Math.sin(i) * _distance;
  points.push(makeBox(x, y));
}

var GRAVITATIONAL_POINT = {
  cpos: (0, _v.v2)(cvs.width / 2, cvs.height / 2),
  ppos: (0, _v.v2)(cvs.width / 2, cvs.height / 2),
  acel: (0, _v.v2)(),
  mass: 100000
};
...
(function step() {
  ...
  for (var i = 0; i < points.length; i++) {
    var point = points[i];
    var dist = (0, _v.distance)(point.cpos, GRAVITATIONAL_POINT.cpos);
    dist > 100 && (0, _gravitation2d2.default)(point, point.mass, GRAVITATIONAL_POINT, GRAVITATIONAL_POINT.mass);
  }
...
function copyCollisionData(output, input) {
  output.resolve = (0, _v.v2)();
  output.hitPos = (0, _v.v2)();
  output.normal = (0, _v.v2)();
  _v.v2.copy(output.resolve, input.resolve);
  _v.v2.copy(output.hitPos, input.hitPos);
  _v.v2.copy(output.normal, input.normal);
  return output;
}
