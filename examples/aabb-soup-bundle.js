...
for (var count = 30, i = 0; i < count; i++) {
  ...
  var cos = Math.cos(i);
  var sin = Math.sin(i);
  var x = centerX + cos * _distance;
  var y = centerY + sin * _distance;
  points.push(makeBox(x, y));
}

var GRAVITATIONAL_POINT = {
  cpos: (0, _index.v2)(cvs.width / 2, cvs.height / 2),
  ppos: (0, _index.v2)(cvs.width / 2, cvs.height / 2),
  acel: (0, _index.v2)(),
  mass: 100000
};
var running = true;
...
(function step() {
  for (var _i = 0; _i < points.length; _i++) {
    var point = points[_i];
    var dist = (0, _index.distance)(point.cpos, GRAVITATIONAL_POINT.cpos);
    dist > 100 && (0, _index.solveGravitation)(point, point.mass, GRAVITATIONAL_POINT, GRAVITATIONAL_POINT.mass);
  }
  ...
  for (var _i3 = 0; _i3 < points.length; _i3++) {
    for (var j = _i3 + 1; j < points.length; j++) {
      var box1 = points[_i3];
      var box2 = points[j];
...
function makeBox(x, y) {
  return {
    ...
    cpos: (0, _index.v2)(x, y),
    ppos: (0, _index.v2)(x, y),
    acel: (0, _index.v2)(),
    mass: 10,
    w: 10,
    h: 10
