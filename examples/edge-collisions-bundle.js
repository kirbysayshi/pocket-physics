...
var GRAVITATIONAL_POINT = {
  cpos: (0, _index.copy)((0, _index.v2)(), CENTER),
  ppos: (0, _index.copy)((0, _index.v2)(), CENTER),
  acel: (0, _index.v2)(),
  radius: 20,
  mass: 1000000
};
...
var boxes = generateBoxes(CENTER, RADIUS, 4);
var points = boxes.reduce(function (all, box) {
  all.push.apply(all, _toConsumableArray(box.points));
  return all;
}, []);
var colliding = [];
...
(function step() {
  ...
  for (var i = 0; i < CONSTRAINT_ITERATIONS; i++) {
    for (var _i = 0; _i < boxes.length; _i++) {
      ...
      var _points = box.points,
          goals = box.goals; // p1, p1mass, p2, p2mass, goal

      (0, _index.solveDistanceConstraint)(_points[0], _points[0].mass, _points[1], _points[1].mass, goals[0]);
      (0, _index.solveDistanceConstraint)(_points[1], _points[1].mass, _points[2], _points[2].mass, goals[1]);
      (0, _index.solveDistanceConstraint)(_points[2], _points[2].mass, _points[3], _points[3].mass, goals[2]);
      (0, _index.solveDistanceConstraint)(_points[3], _points[3].mass, _points[0], _points[0].mass, goals[3]);
      (0, _index.solveDistanceConstraint)(_points[0], _points[0].mass, _points[2], _points[2].mass, goals[4]);
      (0, _index.solveDistanceConstraint)(_points[1], _points[1].mass, _points[3], _points[3].mass, goals[5]);
    }
  }
  ...
  for (var _i2 = 0; _i2 < points.length; _i2++) {
    ...
    if (point !== GRAVITATIONAL_POINT) {
      (0, _index.solveGravitation)(point, point.mass, GRAVITATIONAL_POINT, GRAVITATIONAL_POINT.mass); //sub(force, GRAVITATIONAL_POINT.cpos, point.cpos);
  ...
  for (var _i3 = 0; _i3 < colliding.length; _i3 += 2) {
    var pointA = colliding[_i3];
    var pointB = colliding[_i3 + 1];
    (0, _index.collideCircleCircle)(pointA, pointA.radius, pointA.mass, pointB, pointB.radius, pointB.mass, false, DAMPING);
  }

  collisionEdges(false, points, boxes);

  for (var _i4 = 0; _i4 < points.length; _i4++) {
    var _point = points[_i4];
    (0, _index.inertia)(_point);
  }

  for (var _i5 = 0; _i5 < colliding.length; _i5 += 2) {
    var _pointA = colliding[_i5];
    var _pointB = colliding[_i5 + 1];
    (0, _index.collideCircleCircle)(_pointA, _pointA.radius, _pointA.mass, _pointB, _pointB.radius, _pointB.mass, true, DAMPING);
  }
...
function collisionPairs(pairs, points) {
  ...
  for (var i = 0; i < points.length; i++) {
    ...
    for (var j = i + 1; j < points.length; j++) {
      ...
      if ((0, _index.overlapCircleCircle)(pointA.cpos.x, pointA.cpos.y, pointA.radius, pointB.cpos.x, pointB.cpos.y, pointB.radius)) {
...
function collisionEdges(preserveInertia, points, boxes) {
  ...
  for (var i = 0; i < boxes.length; i++) {
    ...
    for (var j = 0; j < points.length; j++) {
      ...
      (0, _index.rewindToCollisionPoint)(point, box.points[0], box.points[1]);
      (0, _index.rewindToCollisionPoint)(point, box.points[1], box.points[2]);
      (0, _index.rewindToCollisionPoint)(point, box.points[2], box.points[3]);
      (0, _index.rewindToCollisionPoint)(point, box.points[3], box.points[0]);
      (0, _index.collideCircleEdge)(point, point.radius, point.mass, box.points[0], box.points[0].mass, box.points[1], box.points[1].mass, preserveInertia, DAMPING);
      (0, _index.collideCircleEdge)(point, point.radius, point.mass, box.points[1], box.points[1].mass, box.points[2], box.points[2].mass, preserveInertia, DAMPING);
      (0, _index.collideCircleEdge)(point, point.radius, point.mass, box.points[2], box.points[2].mass, box.points[3], box.points[3].mass, preserveInertia, DAMPING);
      (0, _index.collideCircleEdge)(point, point.radius, point.mass, box.points[3], box.points[3].mass, box.points[0], box.points[0].mass, preserveInertia, DAMPING);
    }
  }
}
...
function render(boxes, points, ctx) {
  ...
  for (var _i6 = 0; _i6 < boxes.length; _i6++) {
    var box = boxes[_i6];
